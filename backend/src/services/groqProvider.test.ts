 ...
 * no real Groq key and no network traffic. These tests prove the adapter:
 *
 *   1. POSTs the correct Groq chat-completions request (URL, auth header,
 *      body) from the provider-agnostic request.
 *   2. Maps tool metadata into the Groq `tools` array.
 *   3. Maps a text-only response to `AgentResponse { text }`.
 *   4. Maps tool_calls into provider-independent `AgentToolCall` intents.
 *   5. Supplies prior tool results as Groq `tool` messages for the next round.
...
function stubFetch(
  ...
  const fetch: FetchLike = async (input, init) => {
    calls.push({
      url: String(input),
      init: init as unknown as GroqFetchInit,
    });
...
function makeProvider(body: unknown, status = 200) {
  ...
  const provider = createGroqProvider({ apiKey: API_KEY, model: MODEL });
...
function expectGroqBody(init: GroqFetchInit): Record<string, unknown> {
...
describe("createGroqProvider — request construction", () => {
  it("POSTs to the Groq chat completions endpoint with the auth header", async () => {
  ...
  it("includes the configured model and the user message in the body", async () => {
    ...
    const call = calls[0];
    if (!call) return;
    const body = expectGroqBody(call.init);
  ...
  it("maps tool metadata into the Groq tools array", async () => {
    const { provider, calls } = makeProvider({
      choices: [{ message: { content: "ok" } }],
    });
    await provider.generate(baseRequest());
  ...
  it("supplies prior tool results as Groq tool messages in order", async () => {
  ...
  it("buffers prior-conversation history between system and the current user message", async () => {
    ...
    const call = calls[0];
    if (!call) return;
    const messages = expectGroqBody(call.init).messages as Array<{
      role: string;
      content?: string;
      tool_calls?: unknown;
    }>;
  ...
  it("omits prior history when the request carries no history", async () => {
    ...
    const call = calls[0];
    if (!call) return;
    const messages = expectGroqBody(call.init).messages as Array<{ role: string }>;
    const roles = messages.map((m) => m.role);
...
describe("createGroqProvider — response mapping", () => {
  ...
  it("maps Groq tool_calls into provider-independent intents", async () => {
    const response = await generateProvider({
      choices: [
        {
          message: {
  ...
  it("assigns a fallback id when Groq omits a tool call id", async () => {
    const response = await generateProvider({
      choices: [
        {
          message: {
...
describe("createGroqProvider — output validation", () => {
...
describe("createGroqProvider — HTTP and transport error mapping", () => {
...
describe("createGroqProvider — construction and contract", () => {
  it("throws Authentication when no API key is configured", () => {
    expect(() =>
      createGroqProvider({ apiKey: "", model: MODEL }),
    ).toThrowError(ProviderError);
    expect(() => createGroqProvider({ apiKey: "", model: MODEL })).toThrowError(
